...
export default function Sidebar() {
  ...
  const tabs = useMemo<Tab[]>(() => {
    ...
    // Jobs is shared, but opens the highest authorized operational surface.
    const jobsHref = driver && !dispatch && !admin
      ? `/driver/${modeSlug}`
      : dispatch && !admin
        ? `/dispatch/${modeSlug}`
        : admin
    ...
    const reportsHref = driver && !dispatch && !admin
      ? '/driver/reports'
      : dispatch && !admin
    ...
    const out: Tab[] = [
      {
        ...
        match: p => p.startsWith('/driver/dump-truck') || p.startsWith('/driver/water-truck') || p.startsWith('/driver/hotshot') || p.startsWith('/driver/otr') || p.startsWith('/driver/regional') || p.startsWith('/driver/local') || p.startsWith('/driver/business-vehicle') || p.startsWith('/dispatch/') || /^\/admin\/(dump-truck|water-truck|hotshot|otr|regional|local|business-vehicle)(\/|$)/.test(p),
      },
    ]
    ...
    if (dispatch || admin) {
      out.push({ key: 'assets', label: 'Assets', href: '/admin/equipment', icon: Truck, match: p => p.startsWith('/admin/equipment') || p.startsWith('/maintenance') })
    ...
    <aside className="app-sidebar" style={{
      ...
      <nav style={{ flex: 1, padding: '.75rem .5rem', display: 'flex', flexDirection: 'column', gap: 7, overflowY: 'auto' }}>
        {tabs.map(({ key, label, href, icon: Icon, match }) => {
          ...
          return <Link key={key} href={href} title={collapsed ? label : undefined} style={{
            display: 'flex', alignItems: 'center', gap: 10, padding: '.78rem .82rem', borderRadius: 11,
            textDecoration: 'none', color: active ? 'var(--primary)' : 'var(--muted)',
            background: active ? 'rgba(0,232,176,.1)' : 'transparent',
            border: active ? '1px solid rgba(0,232,176,.16)' : '1px solid transparent',
            fontWeight: active ? 850 : 650, whiteSpace: 'nowrap', overflow: 'hidden',
          }}>
        ...
        {!!identity?.portals.admin && <Link href="/settings" style={{
          display: 'flex', alignItems: 'center', gap: 10, padding: '.78rem .82rem', borderRadius: 11,
          textDecoration: 'none', color: path.startsWith('/settings') ? 'var(--primary)' : 'var(--muted)',
          marginTop: 'auto', fontWeight: 650,
        }}>
      ...
      <div style={{ padding: '.6rem .5rem .85rem', borderTop: '1px solid var(--border)', display: 'grid', gap: 6 }}>
        ...
        <button onClick={signOut} style={{ border: 0, background: 'transparent', color: 'var(--muted)', display: 'flex', gap: 8, alignItems: 'center', justifyContent: collapsed ? 'center' : 'flex-start', padding: '.6rem .75rem', cursor: 'pointer' }}>
          <LogOut size={15} />{!collapsed && <span style={{ fontSize: '.72rem', fontWeight: 750 }}>Log Out</span>}
        </button>
