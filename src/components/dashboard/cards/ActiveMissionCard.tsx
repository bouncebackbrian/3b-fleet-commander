        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {mission ? (
            <>
              ...
              {onTrailerHook && (
                <button onClick={onTrailerHook} style={{ padding: '.5rem .9rem', borderRadius: 10, border: '1px solid rgba(0,232,176,.3)', background: 'rgba(0,232,176,.07)', fontSize: '.82rem', color: 'var(--primary)', fontWeight: 700, cursor: 'pointer', minHeight: 40, display: 'flex', alignItems: 'center', gap: 5 }}>
                  🔗 Hook/Drop
                </button>
              )}
              {onTrailerHistory && (
                <button onClick={onTrailerHistory} style={{ padding: '.5rem .9rem', borderRadius: 10, border: '1px solid var(--border)', background: 'none', fontSize: '.82rem', color: 'var(--muted)', fontWeight: 700, cursor: 'pointer', minHeight: 40, display: 'flex', alignItems: 'center', gap: 5 }}>
                  🚛 Trailer Log
                </button>
              )}
              {onShowHistory && (
