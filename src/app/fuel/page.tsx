...
export default function Fuel() {
  ...
  useEffect(() => {
    fetch('/api/fleet/fuel')
      ...
      .then(data => {
        setEntries(data?.entries ?? SAMPLE_FUEL)
        setLoading(false)
      })
      .catch(() => { setEntries(SAMPLE_FUEL); setLoading(false) })
  }, [])
  ...
  async function handleDelete(entry: FuelEntry) {
    await fetch(`/api/fleet/fuel/${entry.id}`, { method: 'DELETE' })
