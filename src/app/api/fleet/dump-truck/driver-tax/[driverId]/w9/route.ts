/**
 * POST /api/fleet/dump-truck/driver-tax/[driverId]/w9 — submit/update a
 * driver's W-9 (multipart/form-data: signature file + form fields). The
 * driver can submit their own; an admin can also fill one in on their
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ driverId: string }> }) {
  ...
  try {
    ...
    const doc = await uploadDocument({
      ...
      linkedEntityType: 'driver_tax_profile', linkedEntityId: driverId,
      fileName: `w9-signature-${driverId}.png`, mimeType: signature.type || 'image/png', bytes: sigBytes,
      capturedAt: new Date().toISOString(),
    }, auth.userId, auth.email)

    const businessName = form.get('businessName')
    const profile = await submitW9(auth.businessId, driverId, {
    ...
    console.error('[api/fleet/dump-truck/driver-tax/[driverId]/w9] POST error:', err)
