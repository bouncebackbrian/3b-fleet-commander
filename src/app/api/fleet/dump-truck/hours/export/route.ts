...
const EXTRA_PAID_NOTE_THRESHOLD_HOURS = 0.5
...
export async function GET(request: NextRequest) {
  ...
  if (rangeType === 'custom' && (!from || !to)) return NextResponse.json({ error: 'from and to are required when range=custom' }, { status: 400 })
  ...
  try {
    ...
    if (exportType === 'summary') {
      ...
      const { data: equipment } = truckIds.length ? await fleetServiceClient.from('fleet_equipment').select('id, unit_number').in('id', truckIds) : { data: [] as { id: string; unit_number: string }[] }
      const unitByTruckId = new Map((equipment ?? []).map(e => [e.id, e.unit_number]))
      defectRows = defects.map(d => ({ reportedAt: d.createdAt, severity: d.severity, description: d.description, status: d.status, truckUnit: unitByTruckId.get(d.truckId) ?? null, resolvedAt: d.resolvedAt }))
    }
          ...
          sections: [
            {
              ...
              headers: ['Date', 'Truck', 'Total Hrs', 'Broker Hrs', 'Paid Hrs', 'Extra Paid', 'Reg', 'OT', 'Loads', 'Note'],
              rows: rows.map(r => {
                const extra = Math.max(0, r.paidHours - r.customerBillableHours)
                return [r.workDate, r.truckUnit ?? '—', r.totalShiftHours.toFixed(2), r.customerBillableHours.toFixed(2), r.paidHours.toFixed(2), extra.toFixed(2), r.regularHours.toFixed(2), r.overtimeHours.toFixed(2), r.loadsCompleted, extra > EXTRA_PAID_NOTE_THRESHOLD_HOURS ? 'NOTE REQUIRED — extra paid time > 30 min' : '']
              }),
            },
            ...(extraPaidExceptions.length > 0 ? [{
              title: 'Additional Paid Operational Time Over 30 Minutes',
              headers: ['Date', 'Truck', 'Broker Hrs', 'Paid Hrs', 'Extra Paid', 'Required Note'],
              rows: extraPaidExceptions.map(({ row, extra }) => [row.workDate, row.truckUnit ?? '—', row.customerBillableHours.toFixed(2), row.paidHours.toFixed(2), extra.toFixed(2), 'Document assigned driving / yard / shop / fueling / other company-directed work']),
            }] : []),
            {
              title: 'Truck Issues Reported',
              headers: ['Reported', 'Truck', 'Severity', 'Description', 'Status', 'Resolved'],
              rows: defectRows.map(d => [new Date(d.reportedAt).toLocaleString(), d.truckUnit ?? '', d.severity, d.description, d.status, d.resolvedAt ? new Date(d.resolvedAt).toLocaleString() : '']),
            },
          ],
        })
