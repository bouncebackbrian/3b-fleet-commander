...
import type { Load, FuelEntry } from '@/types'
...
export default function MIS() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/fleet/loads').then(r => r.ok ? r.json() : Promise.reject(r)),
      fetch('/api/fleet/fuel').then(r => r.ok ? r.json() : Promise.reject(r)),
    ])
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .then(([l, f]: [any[], any[]]) => {
        setLoads(l as Load[])
        setFuel(f as FuelEntry[])
      })
      .catch(() => { setLoads(SAMPLE_LOADS); setFuel(SAMPLE_FUEL) })
