/**
 * auth-client.ts — Core_Eco Supabase auth client (browser)
 *
 * Connects to Core_Eco Supabase (rkwdryneutgyqrnbuwaz) for identity/session.
 *
 * Core_Eco owns:  auth, sessions, profiles, 3B Account ID, entitlements.
 * Fleet owns:     fleet data only (supabase-browser.ts / supabase-server.ts).
 *
 ...
 * Remove the fallback once NEXT_PUBLIC_CORE_ECO_* are confirmed in all envs.
 */
